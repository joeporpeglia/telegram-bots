import { PointsTable } from "~/pointz-bot/PointsTable.server";
import { isMessageUpdate, isTextMessage } from "~/telegram/utils";
import { FirebaseEventStore } from "~/pointz-bot/EventStore.server";
import {
  Message,
  ChatMember,
  Update,
...
export const action: ActionFunction = async ({ request }) => {
  ...
  const table = PointsTable.create();
  const eventStore = await FirebaseEventStore.create();
  ...
  async function handleAssignPoints(message: Message.TextMessage) {
    ...
    if (sender?.id === recipient?.id && amount > 0) {
      ...
      await table.assignPointsToUser(
        message.chat.id,
        recipient?.id as number,
        amount
      );
      await eventStore.saveEvent(message.chat.id + "", {
  ...
  async function handleListPoints(message: Message) {
    const leaderboard = await table.getPointsLeaderboard(message.chat.id);
  ...
  async function handleEndCompetition(message: Message.TextMessage) {
    ...
    const events = await eventStore.getEvents(message.chat.id + "");
